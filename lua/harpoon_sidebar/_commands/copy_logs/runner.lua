...
local _LOGGER = logging.get_logger("harpoon_sidebar._commands.copy_logs.runner")

local M = {}

--- Modify the user's system clipboard with `result`.
---
---@param result harpoon_sidebar.ReadFileResult The file path + its contents that we read.
---
...
---@class harpoon_sidebar.ReadFileResult
...
---@param callback fun(result: harpoon_sidebar.ReadFileResult): nil Call this once `path` is read.
---@private
---
